...
async def generate_cover(requested_by, title, views, duration, thumbnail):
    ...
    draw.text((205, 550), f"Ad: {title}", (51, 215, 255), font=font)
    draw.text((205, 590), f"Vaxt: {duration}", (255, 255, 255), font=font)
    draw.text((205, 630), f"Izleyici: {views}", (255, 255, 255), font=font)
    draw.text(
        (205, 670),
        f"Başlatan: {requested_by}",
        (255, 255, 255),
        font=font,
    )
    img.save("final.png")
...
async def playlist(client, message):
    ...
    msg = "**İndi Oynanır** in {}".format(message.chat.title)
    msg += "\n- " + now_playing
    msg += "\n- Req by " + by
    temp.pop(0)
    if temp:
...
def r_ply(type_):
    ...
    mar = InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton("Liste 📖", "playlist"),
            ],
...
async def p_cb(b, cb):
    ...
    if type_ == "playlist":
        ...
        now_playing = temp[0][0]
        by = temp[0][1].mention(style="md")
        msg = "<b>İndi Oynayır</b> in {}".format(cb.message.chat.title)
        msg += "\n- " + now_playing
        msg += "\n- Req by " + by
        temp.pop(0)
        if temp:
...
async def m_cb(b, cb):
    ...
    if type_ == "pause":
        ...
        now_playing = temp[0][0]
        by = temp[0][1].mention(style="md")
        msg = "**İndi Oynayır** in {}".format(cb.message.chat.title)
        msg += "\n- " + now_playing
        msg += "\n- Req by " + by
        temp.pop(0)
        if temp:
...
async def play(_, message: Message):
    ...
    if message.chat.id in DISABLED_GROUPS:
        return    
    lel = await message.reply("🔄 <b>Başlatılır</b>")
    ...
    if audio:
        if round(audio.duration / 60) > DURATION_LIMIT:
            await lel.edit(
                f"❌ Video Çox Böyükdü {DURATION_LIMIT} Dəqiqədən Çoc Videolar Qebul Edilmir!"
            )
            return
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("📖 Siyahı", callback_data="playlist"),
        ...
        try:    
            ...
            if (dur / 60) > DURATION_LIMIT:
                 await lel.edit(f"❌ Video Çox Böyükdü {DURATION_LIMIT} Dəqiqədən Çoc Videolar Qebul Edilmir")
                 return
        except:
            pass        
        dlurl=url
        ...
        try:
            toxxt = "**Hansı Musiqini İstəyirsiz**\n\n"
            j = 0
            useer=user_name
            ...
            while j < 5:
                ...
                toxxt += f" ╚ <b>Vaxt</b> - {results[j]['duration']}\n"
                toxxt += f" ╚ <b>İzleyici</b> - {results[j]['views']}\n"
    ...
    if chat_id in callsmusic.active_chats:
        ...
        await message.reply_photo(
            photo="final.png",
            reply_markup=keyboard,
            caption="▶️ <b>Başladı</b> Başlatan {} FarzMusic 😎".format(
...
async def deezer(client: Client, message_: Message):
    if message_.chat.id in DISABLED_GROUPS:
        return
    global que
    lel = await message_.reply("🔄 <b>Axtarılır</b>")
    ...
    if chat_id in callsmusic.active_chats:
        ...
        await res.edit_text(f"✯{bn}✯=▶️ Başlatılır.....")
...
async def lol_cb(b, cb):
    ...
    if chat_id in callsmusic.active_chats:
        ...
        await b.send_photo(chat_id,
            photo="final.png",
            reply_markup=keyboard,
            caption=f"▶️ <b>Başladı</b> Başlatan {r_by.mention} FarzMusic 😎",
        )
